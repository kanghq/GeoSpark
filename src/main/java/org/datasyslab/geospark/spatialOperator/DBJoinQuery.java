...
import org.datasyslab.geospark.joinJudgement.DBGeometryByPolygonJudgementUsingIndex;
...
import org.datasyslab.geospark.spatialRDD.DBSpatialRDD;
 ...
 * The Class JoinQuery.
 */
public class DBJoinQuery implements Serializable{
	 ...
	 */
	private static JavaPairRDD<Polygon, HashSet<Geometry>> executeSpatialJoinUsingIndex(DBSpatialRDD spatialRDD,DBSpatialRDD queryRDD,boolean considerBoundaryIntersection, int DBNum) throws Exception
	{
      ...
      //  }
		...
		int grids = spatialRDD.spatialPartitionedRDD.getNumPartitions();

		ArrayList paral = new ArrayList<Integer>();
		for(int i = 0; i < grids; i++) {
			paral.add(i);
		}
		SparkContext sc = spatialRDD.spatialPartitionedRDD.context();
		JavaSparkContext jsc = JavaSparkContext.fromSparkContext(sc);
		JavaRDD<Integer> pRDD = jsc.parallelize(paral, grids);

		

        JavaPairRDD<Polygon, HashSet<Geometry>> joinResultWithDuplicates = spatialRDD.spatialPartitionedRDD.mapPartitionsToPair(new DBGeometryByPolygonJudgementUsingIndex(considerBoundaryIntersection, DBNum));
    ...
    public static JavaPairRDD<Polygon, HashSet<Polygon>> SpatialJoinQuery(DBPolygonRDD spatialRDD,DBPolygonRDD queryRDD, boolean useIndex,boolean considerBoundaryIntersection, int DBNum) throws Exception {
        if(useIndex)
        {
        	JavaPairRDD<Polygon, HashSet<Geometry>> joinListResultAfterAggregation = executeSpatialJoinUsingIndex(spatialRDD,queryRDD,considerBoundaryIntersection, DBNum);             
